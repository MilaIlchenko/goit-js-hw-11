import{i as p,S as y}from"./assets/vendor-5b791d57.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))o(e);new MutationObserver(e=>{for(const s of e)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&o(n)}).observe(document,{childList:!0,subtree:!0});function r(e){const s={};return e.integrity&&(s.integrity=e.integrity),e.referrerPolicy&&(s.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?s.credentials="include":e.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(e){if(e.ep)return;e.ep=!0;const s=r(e);fetch(e.href,s)}})();function d(a){return a.hits.map(({largeImageURL:t,webformatURL:r,tags:o,likes:e,views:s,comments:n,downloads:f})=>`<div class="gallery-item>
  ...
  </div>`).join("")}const g="34991157-d38b6fef3b34c3f08b128977b",m="https://pixabay.com/api/",i=document.querySelector(".loader");function h(a){const t=`${m}?key=${g}&q=${a}&image_type=photo&orientation=horizontal&savesearch=true`;return i.style.display="block",fetch(t).then(r=>{if(!r.ok)throw new Error(`Response error ${r.status}`);return r.json()}).then(r=>(i.style.display="none",r.hits.length===0&&p.error({title:"Error",timeout:3e3,position:"bottomRight",message:"Sorry, there are no images matching your search query. Please try again!"}),r)).catch(r=>console.log(`Error: ${r}`))}const u=new y(".gallery div ",{captionsData:"alt",captionDelay:250});console.log(u);const b=document.querySelector(".form"),c=document.querySelector(".gallery"),l=document.querySelector(".loader");b.addEventListener("submit",$);l.style.display="none";function $(a){a.preventDefault(),l.style.display="inline-block",c.innerHTML="";const t=a.currentTarget,r=t.elements.query.value.trim();if(r===""){p.show({title:"Error",color:"yellow",message:"Please search for something"}),l.style.display="none";return}h(r).then(o=>{c.innerHTML=d(o),u.refresh(),t.reset()}).catch(o=>{console.error("Error:",o)})}
